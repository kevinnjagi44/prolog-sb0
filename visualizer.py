...
class Visualizer(object):
    ...
    def plot_legend(self, win, y, legend, long_update=True, **kwargs):
        '''
        ...
        x = self.index.get(
            win, 0)  # dict.get(key, default=None). 返回指定键的值，如果值不在字典中返回default值
        self.viz.line(Y=np.array([y]), X=np.array([x]),
                      win=win,
                      opts=dict(title=win, showlegend=True, legend=legend),
                      update='append' if (x > 0 and long_update) else None,
                      **kwargs)
        self.index[win] = x + 1    # Maintain the X
